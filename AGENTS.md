...
碰撞音频在每次合法投掷/重掷的用户手势栈内先调用 `soundManager.prepare()`，同步创建 AudioContext、触发必要的异步恢复并生成首个碰撞 noise buffer，然后才执行 `throwDice()` 与首个物理步，避免首次碰撞回调把上下文/buffer 初始化成本计入 `world.step()`；静音或 Web Audio 不可用时仍静默降级。当前视觉收口保持原资源预算：rolling 时顶栏关闭大面积背景采样，移除会逐帧重绘阴影的按钮 pulse；移动顶栏、结果卡和桌面侧栏压缩；ResultPanel 的系统“云/兔”字形替换为纯 CSS 细线角纹；海碗与桌面仅改写既有 CanvasTexture/材质参数，不增加 mesh、材质或 texture 数量。
