...
这些命令会先用 `vite build --mode e2e` 构建隔离产物，再由 Playwright 自动启动并停止严格端口的 preview server。浏览器诊断当前使用 schema v8，必须是带 `schemaVersion / revision / sampleKind: post-render` 的渲染后快照；除 schema v6 已有的 initial-state v1、结构、渲染质量和安全包络外，还发布 scheduler preset/explicit、accumulator 时间分类/守恒、terminal-abandoned/overload、模拟步数/时间与结算后的 canonical final-state v1。初末态都以 IEEE-754 Float64 大端字节的 FNV-1a 64 签名锁定 position/quaternion/velocity/angularVelocity，不得用 `toFixed` 或再次消费随机数重建。`?nextSeed=<整数>` 与带版本的 `nextSeeds` 队列只在开发/e2e 模式生效，强制 timeout outcome 与调度 experiment query 仅在隔离 e2e 构建生效；生产构建忽略调度 query 并固定 `exact-cap6`。
...
clean `e20d359` 的 schema v8 浏览器门禁结果：`test:e2e` 8/8、`bench:browser` 2/2；production-default soak 桌面/移动各 20 轮均 20 natural，耗时 72.794s / 53.628s，最大 terminal queue 86.8993ms / 56.2333ms，最大半径/穿透 `0.6571491133m / 0.0548978013m`，boundary/wall/guard/non-finite/页面错误为 0，资源无增长。legacy rollback soak 2/2 通过。scheduler A/B v2 的 5 seeds 在两端结果/奖级/结算路径/安全等价，但 `trajectoryEquivalentSeedCount=0`；桌面/移动 exact/legacy 的 rAF p95 比值为 `0.986535928581883` / `0.9161676646706578`，wall 比值为 `0.94935576135633` / `1.0074334792048256`，只作 observation-only。生产切换依据是逐步真值、可复现性和异常语义，不得写成普适性能提升或轨迹等价。

手工 Chrome 单轮确认无 query 为 `explicit=false / exact-cap6`：seed 50000 natural-sleep@2.1333s/128 steps，3D 可见面与 UI 同为 `1,5,6,6,4,4`，判“二举，带18”，重置后回第 1 轮且结果/历史清空。该观察不替代批量门禁；真实 OS/tab 隐藏恢复与真实设备 GPU 仍是未完成验收。
