...
- 核心 UI 包括：掷骰按钮、当前轮次、结果面板、累计奖级记录、最近 5 轮历史、重置按钮和音效开关。
...
- timeout 的目标是保证流程有出口，不等于允许读取仍在运动的姿态并提交正常奖级。超时后的重试、救援或失败 UI 应由明确产品策略决定。
...
| 命令                 | 用途                                                                                                           |
| -------------------- | -------------------------------------------------------------------------------------------------------------- |
| `pnpm test:e2e`      | Playwright 桌面/移动端完整一轮、reset、固定 seed 投掷诊断、静态零帧与移动布局验收                              |
| `pnpm bench:browser` | 对 idle/rolling/settled 的 calls、triangles、资源数、DPR/像素预算和静态调度设硬门槛，并输出 JSON/截图 artifact |

两条命令会先用 `vite build --mode e2e` 构建隔离产物，再由 Playwright 自动启动并停止严格端口的 preview server。浏览器诊断必须是带 `schemaVersion / revision / sampleKind: post-render` 的渲染后快照；`?nextSeed=<整数>` 只在开发/e2e 模式生效且只消费一次。当前结构预算集中在 `e2e/helpers/diagnostics.ts`，不要把受机器、浏览器调度和软件渲染影响的 rAF 帧时设为通用硬门槛；它只写入 artifact 供同环境 A/B 对比。
