...
- [ ] 2B.1 [实现] 实现 `ThrowButton.tsx`：掷骰按钮，通过 useGameController().throw() 调用
- [ ] 2B.2 [实现] 掷骰中（rolling phase）显示"骰子翻滚中"并禁用
- [ ] 2B.3 [实现] result phase 恢复可用
- [ ] 2B.4 [实现] 实现 `ResultPanel.tsx`：当轮结果面板
- [ ] 2B.5 [实现] 显示 6 颗骰子点数（数字 + 视觉排列）
- [ ] 2B.6 [实现] 显示奖级名称（大号字，节庆配色）
- [ ] 2B.7 [实现] 显示"带数"值（当奖级有带数时）
- [ ] 2B.8 [实现] 未中奖时显示"未中奖"
- [ ] 2B.9 [实现] 实现 `PrizeRecord.tsx`：累计奖级记录面板
- [ ] 2B.10 [实现] 展示本局各奖级获得次数（奖池/榜单风格）
- [ ] 2B.11 [实现] 实现 `History.tsx`：显示最近 HISTORY_MAX_LENGTH 轮历史（值来自 config/ui.ts）
- [ ] 2B.12 [实现] 每条记录含轮次号、点数组合、奖级名称、带数
- [ ] 2B.13 [实现] 实现 `SoundToggle.tsx`：音效开关按钮，通过 useGameController().toggleSound() 调用
- [ ] 2B.14 [实现] 实现 `ResetButton.tsx`：重置按钮，通过 useGameController().reset() 调用；rolling 阶段禁用
- [ ] 2B.15 [实现] 实现当前轮次显示
- [ ] 2B.16 [实现] 所有 UI 组件只通过 selector 读取 store，写入操作通过 useGameController() 获取的 controller 实例

### 2C 样式与主题

- [ ] 2C.1 [实现] 定义 CSS Variables：主色（红）、辅色（金）、底色（米白）、文字色
- [ ] 2C.2 [实现] 结果面板样式：传统节庆风格，红底金字或金边红字
- [ ] 2C.3 [实现] 按钮样式：大尺寸、可触摸、有 hover/active/disabled 状态
- [ ] 2C.4 [实现] 奖级记录面板样式：紧凑列表
- [ ] 2C.5 [实现] 历史记录样式：滚动列表、条目区分
- [ ] 2C.6 [实现] 整体布局：桌面端 canvas 居中 + 右侧/底部 UI 面板
- [ ] 2C.7 [实现] 移动端布局：canvas 上半 + UI 下半，按钮足够大（≥44px touch target）
- [ ] 2C.8 [实现] CSS 媒体查询断点处理（桌面/平板/手机）

### 2D 阶段二集成验证

- [ ] 2D.1 [验收] 桌面端完整操作流程：掷骰 → 翻滚 → 结果展示 → 再次掷骰 → 累计记录更新
- [ ] 2D.2 [验收] 移动端同上流程验证（Chrome DevTools 模拟 + 真机）
- [ ] 2D.3 [验收] 重置功能验证：非 rolling 时清空记录、轮次归 1、骰子复位；rolling 时重置按钮处于禁用状态
- [ ] 2D.4 [验收] 连续 20 轮操作：按钮状态始终正确（掷骰中掷骰按钮和重置按钮均禁用、结束后恢复）
- [ ] 2D.5 [验收] 历史记录正确显示最近 HISTORY_MAX_LENGTH 轮（默认 5）
- [ ] 2D.6 [验收] 奖级记录累加正确
- [ ] 2D.7 [验收] 全部单测通过：`pnpm test`
- [ ] 2D.8 [验收] `pnpm build` 通过，无编译错误
