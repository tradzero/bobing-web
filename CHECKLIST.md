...
- [x] 4.1 workspace 拆分为 web/server/game-domain/protocol/physics-core，保留多房间边界
- [x] 4.2 PostgreSQL 迁移与 repository：房间、成员、游戏、回合、投掷、63 份奖池、领取和状元 claim
- [x] 4.3 默认开放房间加入/会话恢复/首位房主/开局后旁观者语义
- [x] 4.4 WebSocket protocol v1、RoomHub 广播和版本化 RoomSnapshot
- [x] 4.5 服务端共享 headless 物理、seed 广播、客户端统一动画和安全失败不结算
- [x] 4.6 PostgreSQL 绝对 deadline：玩家回合、倾斜确认、结束选择，全部可通过环境变量调整
- [x] 4.7 普通奖项原子扣减、个人/全员领取查看、同玩家最后状元与全局抢状元
- [x] 4.8 奖池清空后由房主选择立即结束或锁定玩家每人加投一次
- [x] 4.9 桌面/移动多人 UI：加入、房主开局、倒计时、奖池、个人奖项、全员数量、最近结果
- [x] 4.10 真实 PostgreSQL 集成测试覆盖恢复、房主权限、超时换人、权威揭晓/扣奖和倾斜重投
- [ ] 4.11 补多浏览器同时在线的自动化 WebSocket/e2e 门禁和断线重连 soak
- [ ] 4.12 将当前 `physics-core` 过渡 façade 下的无 DOM 模块机械迁入独立 package，去除 server tsconfig 的 DOM lib
- [x] 4.13 本局结束后由原房主按锁定阵容再开一局，结束后新加入者保持旁观
- [ ] 4.14 实现房间列表/创建/密码校验；不得改变 protocol v1 已有单房间契约
- [x] 4.15 提供需匹配房间 ID 二次确认的强制重置 CLI；保留房间配置，事务清空成员和全部对局数据
- [x] 4.16 浏览器以同源 localStorage 绑定昵称/恢复令牌；普通重启自动重连，清房后保留昵称并自动重绑
