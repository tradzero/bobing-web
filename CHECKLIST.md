...
- [x] 根路径开放房大厅、默认房列表入口、服务端创建 ID 和可分享的动态房间 URL
...
- [x] 按房间隔离的浏览器 localStorage 恢复令牌、重启/断线恢复和清房后自动重绑
- [x] 二次确认的停服强制重置 CLI
- [x] 真实 PostgreSQL 集成测试覆盖关键事务语义（只在显式测试库运行）
- [x] 持久化房间生命周期：空房删除、闲置对局废弃、归档隐藏、延迟硬删除和默认房保留
...
- [x] 四 BrowserContext 真实 WebSocket/PostgreSQL E2E：大厅创建/发现两个房间、分别加入/开局/权威投掷、广播隔离、轮次交接和刷新恢复
- [x] PostgreSQL 集成测试覆盖跨房间状态隔离，以及并发重放同一投掷命令只生成一个权威结果
- [x] 多人 E2E 定向中断真实 WebSocket，验证按房间恢复令牌自动重连
- [x] 多人 E2E 通过第二条真实连接重放相同 UUID，验证连接层只提交一次投掷
- [x] 多人 E2E 真实停止/重启服务进程，验证身份恢复和 PostgreSQL 逾期回合推进
- [x] 扩展多人 E2E：整局奖池耗尽、立即结束/加投、最终状元和同阵容再开一局
- [x] 三浏览器多人 soak：真实投掷、幂等/奖池守恒、中途恢复、连续 deadline 和多局重开
- [x] 密码房创建/验证和访问控制；保持 protocol v1 默认房间与开放房兼容
- [x] 无真实操作的进行中对局转为 `abandoned`，终止当前 turn 且不再生成 deadline 写入
- [x] 默认房永久；空房 1h 删除、闲置房 24h 归档、归档 7d 删除，全部支持环境变量调整
- [x] 提供恢复令牌/WebSocket session 鉴权的房主主动关闭/归档入口；自动清理不依赖当前连接数
- [ ] 明确离房、踢人、房主转移和座位回收语义后，再提供切换身份入口
- [ ] 公网部署暂不在当前阶段：离开本地 LAN 前补 TLS、认证、速率限制和可信代理边界
