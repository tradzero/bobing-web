...
- 已完成桌面程序化木纹与海碗第一轮低 GPU 收口；海碗现试用单张 WebP 青花纹样，解码并完成首帧纹样渲染前显示加载页，失败或 8 秒超时回退程序化纹样，几何/材质/纹理数量不变
...
clean `e20d359` 的 schema v8 浏览器门禁保留为迁移前基线：`test:e2e` 8/8、`bench:browser` 2/2、显式 legacy rollback soak 2/2；scheduler A/B v2 的 5 seeds 在桌面/移动保持结果、奖级、结算路径和安全等价，但轨迹与性能都只作限定观察。当前美术/音频/profile 变更对应 clean checkpoint `1060095c10c1d688ba1f1855f2f00ba799179407`：schema v9 production-default soak start/end clean unchanged，桌面/移动各 20/20 natural，耗时 66.222s / 57.235s，最大 terminal queue 97.3ms / 59.7667ms，最大半径 `0.6571491133m`、最大接触穿透 `0.0548978013m`，安全/页面错误为 0，WebGL 资源保持 `1 canvas / 8 geometries / 6 textures / 10 warmed programs`。这些 wall time 只作同环境观察，不宣称通用 FPS。
