/**
 * dice-dice contact-equation 参数 sweep
 *
 * 目标：验证低速黏连 / 尾段稳定窗口反复打破是否主要由接触方程收敛造成。
 */
...
const VARIANTS: Variant[] = [
  ...
  {
    label: 'softer-both',
    diceDiceContactEquationStiffness: 2e6,
    diceDiceContactEquationRelaxation: 6,
    diceDiceFrictionEquationStiffness: 2e6,
    diceDiceFrictionEquationRelaxation: 6,
  },
]
...
for (const variant of VARIANTS) {
  const results = seeds.map((seed) => ({
    ...
    result: runTrial({
      ...
      diceDiceFrictionEquationStiffness: variant.diceDiceFrictionEquationStiffness,
      diceDiceFrictionEquationRelaxation: variant.diceDiceFrictionEquationRelaxation,
    }),
  }))
  ...
  console.log(
    `${variant.label.padEnd(14)}| ${String(timeoutCount).padStart(3)}/${String(seeds.length).padEnd(3)} | ` +
