...
import { bowlFloorMaterial, bowlWallMaterial, tableMaterial, setupContactMaterials } from '@/physics/materials'
...
function createBowlBodiesWithGrid(world: CANNON.World, gridSize: number) {
  ...
  for (let i = 0; i < WALL_COUNT; i++) {
    ...
    wall.addShape(new CANNON.Box(new CANNON.Vec3(wallTangentialHalf, WALL_HEIGHT / 2, WALL_THICKNESS / 2)))
    wall.quaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), angle)
    world.addBody(wall)
  }

  // 桌面
  const table = new CANNON.Body({ mass: 0, material: tableMaterial, position: new CANNON.Vec3(0, -0.075, 0) })
...
function runSeed(seed: number, gridSize: number): SeedResult {
  ...
  for (let f = 0; f < maxFrames; f++) {
    ...
    const allBelow = bodies.every(
      b => b.velocity.length() < SETTLE.speedThreshold &&
    ...
    if (allBelow) {
      ...
      if (stableStart >= 0) { stableBrokenCount++; stableStart = -1 }
    }
...
for (const grid of GRID_SIZES) {
  ...
  console.log(`  性能: avg=${avgStepTime.toFixed(0)}ms, max=${maxStepTime.toFixed(0)}ms (per seed full sim)`)
...
for (const grid of GRID_SIZES) {
  ...
  console.log(`  grid=${grid}: maxYRise=${(r.maxYRise * 1000).toFixed(1)}mm, broken=${r.stableBroken}, sleep=${r.sleepTime >= 0 ? r.sleepTime.toFixed(2) + 's' : 'N/A'}, step=${r.stepTimeMs.toFixed(0)}ms`)
}
