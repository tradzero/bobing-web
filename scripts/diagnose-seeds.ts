import { createPhysicsWorld } from '../apps/web/src/physics/world.ts'
import { createBowlBodies, ESCAPE_Y } from '../apps/web/src/physics/bowl-body.ts'
import { setupContactMaterials, diceMaterial } from '../apps/web/src/physics/materials.ts'
import { PHYSICS } from '../apps/web/src/config/physics.ts'
import { SETTLE } from '../apps/web/src/config/settle.ts'
import { reseed } from '../apps/web/src/utils/random.ts'
import { throwDice } from '../apps/web/src/dice/throw.ts'
import { readAllFacesDetailed } from '../apps/web/src/dice/read-face.ts'
import type { DicePair } from '../apps/web/src/dice/create.ts'
...
for (const seed of [1776310976115, 1776311021115]) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const v = b.velocity.length(),
      av = b.angularVelocity.length()
    const tilt = d.confidence < SETTLE.tiltThreshold ? ' ⚠TILT' : ''
    console.log(
      `  die${i + 1} val=${d.value} conf=${d.confidence.toFixed(3)} angle=${angle.toFixed(1)}° r=${r.toFixed(3)} y=${b.position.y.toFixed(3)} sleep=${sleeping} v=${v.toFixed(4)} av=${av.toFixed(4)}${tilt}`,
    )
  }
  ...
  for (let i = 0; i < 6; i++) {
    for (let j = i + 1; j < 6; j++) {
      ...
      if (dist < 0.4) console.log(`    die${i + 1}-die${j + 1} dist=${dist.toFixed(4)}`)
    }
  }
  dispose()
}
