...
import { BOWL_THICKNESS, BOWL_HEIGHT, BOWL_INNER_RADIUS, sampleBowlInnerProfile } from '../src/config/bowl.ts';
...
for (let i = 0; i < points.length; i++) {
  for (let j = i + 1; j < points.length; j++) {
    if (Math.abs(points[i].x - points[j].x) < 1e-6 && Math.abs(points[i].y - points[j].y) < 1e-6) {
      console.log(`  重复: [${i}] 和 [${j}]: (${points[i].x.toFixed(6)}, ${points[i].y.toFixed(6)})`);
      dupes++;
    }
  }
}
...
for (let i = 0; i < pos.count; i++) {
  ...
  if (r < 0.01) {
    ...
    if (centerVerts < 8) console.log(`  v[${i}] pos=(${x.toFixed(4)},${y.toFixed(4)},${z.toFixed(4)}) normal=(${nx.toFixed(3)},${ny.toFixed(3)},${nz.toFixed(3)})`);
    centerVerts++;
  }
}
...
let bottomFaces = 0;
let nyPositive = 0, nyNegative = 0, nyZero = 0;
...
for (let col = 0; col < Math.min(cols, 8); col++) {
  ...
  if (vi < pos.count) {
    ...
    console.log(`  col=${col} r=${r.toFixed(4)} normal=(${nx.toFixed(3)},${ny.toFixed(3)},${nz.toFixed(3)})`);
...
if (lastRowNormals.length > 1) {
  ...
  for (let i = 1; i < lastRowNormals.length; i++) {
    const a = lastRowNormals[0], b = lastRowNormals[i];
